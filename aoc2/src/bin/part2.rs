...
impl Game {
    ...
    fn is_game_possible(&self) -> bool {
        ...
        let max_scores = self.get_fewest_possible_cubes();

        return max_scores.red <= MAX_POSSIBLE_RED
            && max_scores.green <= MAX_POSSIBLE_GREEN
            && max_scores.blue <= MAX_POSSIBLE_BLUE;
    }

    fn get_fewest_possible_cubes(&self) -> Scores {
        let mut max_red = 0;
        let mut max_green = 0;
        let mut max_blue = 0;
    ...
    fn get_power_of_cubes(&self) -> i32 {
        let cubes = self.get_fewest_possible_cubes();
...
fn part1(input: &str) -> i32 {
    ...
    let sum = calculate_possible_game_sum(data.clone());
    println!("Sum: {}", sum);
    let power = calculate_power_sum(data);
    println!("Power: {}", power);
    return power;
}
...
fn calculate_power_sum(games: Vec<Game>) -> i32 {
    let count: i32 = games.iter().map(|game| game.get_power_of_cubes()).sum();
    return count;
}
